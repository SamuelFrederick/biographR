...
It would be challenging, if not impossible, to extract structured biographical data about John Smith from this text without reading the text and getting the information by hand. Instead, we can use the `get_bio()` function from the `biographR` package, which calls ChatGPT's API to extract the data for us. 
...
We can also customize the output further using the `prompt_fields_format` and `prompt_fields_values` arguments. Additionally, while the package defaults to ChatGPT 3.5, ChatGPT 4 often appears to perform somewhat better in extracting subtler information (e.g., gender from pronouns).
